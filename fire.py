...
# === VIDEO MODE ===
    ...
    if uploaded_video:
        ...
        if action == "Run Detection":
            ...
            # Interactive slider
            frame_number = st.slider("Select frame", 0, total_frames - 1, step=1)
            ...
            if ret:
                ...
                st.image(frame_rgb, caption=f"Frame {frame_number + 1}", use_column_width=True)
            ...
            st.markdown("---")
            st.markdown("### Original Video Playback (No Detection Overlay)")
